...
const SkillToolbar: FC<ChatInputProps> = ({
}) => {
    ...
    return renderSkillToolbar()
}

const ActionsToolbar: FC<ChatInputProps> = ({
    feedbackDisabled,
    visionConfig
}) => {
    ...
    const renderActions = () => {
        ...
        return (
            <div className='flex justify-center items-center gap-3 w-full absolute bottom-full'>
                {actions.map((action, index) => (
                    <div
                        key={index}
                        className='h-[32px] flex justify-center items-center rounded-[12px] px-3 bg-gray-100 cursor-pointer hover:bg-gray-200'
                    >
...
const SkillToolbarComponent = React.memo(SkillToolbar);
const ActionsToolbarComponent = React.memo(ActionsToolbar);
const ChatToolbarComponent = React.memo(ChatToolbar);
